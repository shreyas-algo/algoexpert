def spiralTraverse(array):
    ...
    while spiral_length < matrix_len:
        ...
        if direction == "right" and exceedingRight(col, right_boundary):
            direction, row, col = pivotDown(row, col, direction, right_boundary, bottom_boundary)
            
        if direction == "down" and exceedingDown(row, bottom_boundary):
            direction, row, col = pivotLeft(row, col, direction, bottom_boundary, left_boundary)
            
        if direction == "left" and exceedingLeft(col, left_boundary):
            direction, row, col = pivotTop(row, col, direction, left_boundary, top_boundary)
        
        if direction == "top" and exceedingTop(row, top_boundary):
            direction, row, col = pivotRight(row, col, direction, top_boundary, right_boundary)
