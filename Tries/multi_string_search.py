...
# Analysis: Creation: O(m) where m is the length of the longest word & O(n) space where n is the length of the bigString
    ...
    def contains(self, string):
        ...
        for char in string:
            if char not in current_dict:
                return False
            current_dict = current_dict[char]
        # Following commented because substrings of a word considered okay eg "test" is considered contained in "these are tests" [discuss with interviewer]
        # Uncomment when you want exact string match
