# Approach I: Brute Force: for every index, go over the hops and see if items repeat
# O(N*N) time - for every item, you'll traverse the whole array in case if it's a single cycle array (True case)

# Approach II: Optimization: Think about it. You dont actually need to check all indexes. if you start from 0 and reach 0 without any repetition - that means the path is a single cycle which does not repeat any index. Which means no matter where you start you will have a successful single cycle. If there was a cycle in between, you would encounter some element twice before reaching 0
# O(N) time & O(N) space
...
#  Approach II: Optimized implementation
def hasSingleCycle(array):
    ...
    count = 0
    while True:
        # if you're back to 0 and you traversed whole array elements (count check) -- then it's a single cycle input -- by definition. Look at question description
